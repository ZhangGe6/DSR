...
def save_network(model,optimizer, epoch, loss):
    save_path = '../model_ckpts/PCB/share/epoch_{} loss_{}'.format(epoch, loss)+'.pth'
...
if opt.PCB:
    print('PCB')
    model = PCB(len(class_names), mode='PCB_share_fc')
if use_gpu:
    model = model.cuda()
...
if not opt.PCB:
    ...
    ignored_params +=(list(map(id, model.share_classifier.parameters() ))
                        )
    ...
    optimizer_ft = optim.SGD([
             ...
             {'params': model.share_classifier.parameters(), 'lr': opt.lr},
...
#
dir_name = '../model_ckpts/PCB/share/'
if not os.path.isdir(dir_name):
    os.mkdir(dir_name)
#record every run
copyfile('./DSR_PCB.py', dir_name+'/DSR_PCB.py')
...
#---------------------------
def save_network(model,optimizer, epoch, loss):
    save_path = '../model_ckpts/PCB/epoch_{} loss_{}'.format(epoch, loss)+'.pth'
    torch.save({'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer':optimizer.state_dict(),'loss': loss}, save_path)
...
if opt.PCB:
    print('PCB')
    model = PCB(len(class_names), mode='PCB')
if use_gpu:
    model = model.cuda()
