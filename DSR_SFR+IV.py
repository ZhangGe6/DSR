...
def main():
    ...
    model = ft_net(class_num=751, mode='train_SFR+IV')
    if use_gpu:
     model = model.cuda()
    ...
    for ep in range(start_ep, cfg.total_epochs):
        ...
        while not cur_epoch_done:
          ...
          label_pred, globalFeature, spatialFeature = model(ims_var)          
          ...
          loss_id = criterion(label_pred, labels_t)
          ...
          if step % 1 == 0:
            ...
            train_log = (', verif prec {:.2%}, d_ap {:.4f}, d_an {:.4f}, loss_veri {:.4f}, iden prec {:.2%}, loss_id {:.4f},'.format(prec_meter.val,dist_ap_meter.val, dist_an_meter.val,loss_meter.val, corrects/len(labels), loss_id.item()))
...
def save_network(model,optimizer, epoch, loss):
    save_path = '../model_ckpts/SFR+IV/epoch_{} loss_{}'.format(epoch, loss)+'.pth'
